	...
	try {
		...
		if (data) {
			updatedData = await Promise.all(
				data.map(async (farm) => {
					const { data: farmData, error: farmDataError } = await supabase
						.from("cattle")
						.select("*")
						.eq("farm_id", farm.id);

					if (farmDataError) {
						console.error("Error fetching farm data:", farmDataError);
						return farm;
					}

					return { ...farm, cattleList: farmData };
				})
			);
